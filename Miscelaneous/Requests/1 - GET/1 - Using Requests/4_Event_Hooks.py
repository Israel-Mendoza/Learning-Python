...
def response_information(response: requests.models.Response) -> str:
    return (f"Response object @ {hex(id(response)).upper()}\n\t"
            f"URL: {response.url}\n\tStatus code: {response.status_code}")


def my_first_hook(response: requests.models.Response, *args, **kwargs) -> None:
    print(f"Argument passed to the first hook: {response_information(response)}")


def print_request_url(response: requests.models.Response, *args, **kwargs) -> requests.models.Response:
    print(f"Adding hook attribute to {response_information(response)}")
    setattr(response, "hook_called", True)
    return response


my_response: requests.models.Response = requests.get(url, hooks={"response": [my_first_hook, print_request_url]})
# Argument passed to the first hook: Response object @ 0X104FD3F20
# 	URL: http://localhost:8000/api/items
# 	Status code: 200
# Adding hook attribute to Response object @ 0X104FD3F20
# 	URL: http://localhost:8000/api/items
# 	Status code: 200

try:
    print(response_information(my_response))
    print(f"{my_response.hook_called = }")
